...
class Dashboard extends Component {
  ...
  componentDidMount() {
    this.readSummaryMonth();
    this.readRecords();
  }

  readSummaryMonth = () => {
    this.setState( { loading2: true }, () => {
      Api.post('/wp-json/pbadc/v1/get_summary_month', {
        'user_id': localStorage.getItem('userId'),
        'api_key': localStorage.getItem('apiKey'),
        'company_id': localStorage.getItem('companyId')
      }).then( res => {
					if ( 200 === res.status ) {
            this.setState( { loading2: false, dataBar: res.data } );
					}

				} )
				.catch( err => this.setState( { loading2: false, error: err } ) );
		} )
  }
  ...
  render() {
    const { loading, loading2, dataBar, data, info, error, confirmMsgTitle, msgConfirm, func, baseUrl, nPage } = this.state;
